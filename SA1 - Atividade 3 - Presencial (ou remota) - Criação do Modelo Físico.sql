...
create table grupo(
idgrupo varchar(5) not null primary key,
descricao varchar(80) not null,
unique index(idgrupo) 
);

create table estadio(
idestadio int not null auto_increment primary key,
discricao varchar(45) not null,
localidade varchar(45) not null,
capacidade int not null,
unique index(idestadio)
);
...
constraint FK_pais_grupo #nome do relacionamento
foreign key(grupo_idgrupo) #especifica quem será a FK(nesta tabela)
references grupo(idgrupo), #de onde irá vir a PK
unique index(idpais)
);
...
unique index(idjogador)
);


create table jogos(
idrodada int not null auto_increment primary key,
data datetime not null,
estadio_idestadio int not null,
constraint FK_Estadio_jogos
foreign key(estadio_idestadio)
references estadio(idestadio),

pais_idpais_1 int not null,
constraint FK_Pais1_Jogos
foreign key(pais_idpais_1)
references pais(idpais),

pais_idpais_2 int not null,
constraint FK_Pais2_Jogos
foreign key(pais_idpais_2)
references pais(idpais),

gols_idpais_1 int not null,
gols_idpais_2 int not null,
...
amarelo tinyint not null,
vermelho tinyint not null,
tempo varchar(45) 
);

create table substituicao(
jogos_idrodada int not null,
jogos_idjogador_sai int not null,
jogos_idjogador_entra int not null,

constraint FK_Jogos_Substituicao
foreign key (jogos_idrodada)
references jogos(idrodada),

constraint FK_Jogadorsai_Substituicao
foreign key (jogos_idjogador_sai)
references jogador(idjogador),

constraint FK_Jogadorentra_Substituicao
foreign key (jogos_idjogador_entra)
references jogador(idjogador),
...
insert into substituicao values(24,3735,3765,'75:39');
	...
	select 
	pais.selecao, jogador.nome 
	from jogador
	inner join pais
	on pais.idpais = jogador.pais_idpais;

	#Seleção, o nome do jogador e número de vezes que ele foi substituído;
	select 
    pais.selecao,
    jogador.nome, 
    count(substituicao.jogos_idjogador_sai) as Substituicao
	from 
    substituicao 
	inner join jogador  
		on jogador.Idjogador = substituicao.jogos_idjogador_sai
	inner join pais 
		on pais.idPais = jogador.pais_Idpais
	group by jogos_idjogador_sai;
    
    #Número total de cartões amarelos e vermelhos por seleção;
	select 
		pais.selecao, 
		SUM(cartao.amarelo) as amarelo,
		SUM(cartao.vermelho) as vermelho  
	from
		pais, jogador, cartao
	where
		jogador.pais_idpais = Idpais and
		jogador.Idjogador = cartao.jogador_idjogador
	group by pais.selecao;

	#Tabela de pontuação ordenada por grupo 
	select 
		pais.grupo_idgrupo as grupo, 
		pais.selecao, pais.pontos,
		pais.vitorias, pais.empates, 
		pais.derrotas, 
		pais.golspro - pais.golscontra as saldo_gols
	from pais
	order by grupo asc, pontos desc;
